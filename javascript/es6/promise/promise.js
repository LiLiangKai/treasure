/*
Promise有三种状态，等待态（pending）、执行态（fulfilled）和拒绝态（rejected），状态只能从 pending -> fulfilled 或 pending -> reject，且过程不可逆

创建Promise示例
初始化Promise状态为PENDING
立即执行Promise中传入的fn回调函数，Promise内部将resolve和reject作为参数传递给fn，按事件机制处理（then、catch注册回调，resolve、reject触发回调）
执行then注册回调处理数组，then方法返回一个Promise
Promise里的关键是要保证，then方法传入的参数 onFulfilled 和 onRejected回调函数，必须在then方法被调用的那一轮事件循环之后的新执行栈中执行
*/
