...
以上例子中，我们可以把 `customerA` 对象作为访问者对象，`Apple`对象作为原对象，B对象则是代理对象。访问者仅负责发起访问原对象的请求，并接受最终的请求处理结果，而对访问者发起的请求是全权由代理对象负责处理，由代理对象对原对象进行访问操作（该过程对访问者对象来说是透明的），并将访问操作的最终结果返回给访问者对象。
